...
module rail_side(half_width, length, y) {
  translate([-half_width - 2, y, 0]) cube([2, length, 9]);
  hull() {
    translate([-half_width - 0.15, y, 5.5]) rotate([0, -40, 0]) cube([1.5, length, 1]);
    translate([-half_width - 0.5, y, 6.4]) cube([1.5, length, 1]);
  }
  translate([-half_width - 0.35, y, 8]) rotate([0, -40, 0]) cube([1.3, length, 1]);
}
